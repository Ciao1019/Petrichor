...
> 实施状态：服务端公开作用域、Wiki、统一搜索和订阅能力已完整实现。根据 2026-09-03 的产品调整，公开前台恢复为原文章归档界面，暂不展示 Wiki、图谱与统一搜索页面；对应路径统一返回首页，后端能力保留。2026-09-04 进一步下线全站星图（`/graph`）及其后台、API 和 Agent 检索能力；知识库内 Wiki 图谱与文章知识图谱继续保留。
...
- 原 `/graph` 全站星图已于 2026-09-04 下线；`SiteGraphExplorer` 仅继续作为 Wiki 关系图谱的共享渲染器。
...
- 公开文章、Wiki、公开问答和公开 Wiki 图谱各自维护可见性 SQL，后续容易继续漂移。
...
自 2026-09-04 起，`/graph` 全站星图已下线。公开 Wiki 图谱仍放在具体知识库下，文章知识图谱也继续保留。
...
由公开文章、Wiki、公开 Wiki 图谱、问答、搜索和 Feed 共用，避免 `publicapi -> sitecontent` 的依赖环。该包只负责：
