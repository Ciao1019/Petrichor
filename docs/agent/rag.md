...
每次问题生成、候选抽取、目录规划和页面生成模型调用最多尝试 3 次。仅 408/409/425/429、上游
5xx、网络异常和非法 JSON 会按 1 秒、2 秒退避重试；其它供应商 4xx 不重试。重试耗尽后，可降级阶段使用模板问题、
...
- **范围**：已启用、无密码、未过期、未撤销的公开分享文章；Wiki 与目录节点还要求页面所有来源及构建记录均可公开
- **运行方式**：与站内助手同一套 Runtime（意图、复杂度、计划、证据、质量门、Agent 事件），切换为公开档位；单次运行最长 3 分钟
- **检索**：工具与后台同名同契约（`lookup_knowledge` 等），实现复用公开搜索的分片全文与语义召回、RRF 融合；语义不可用时回退全文；尚未构建索引的文章保留原文关键词检索；深读时在公开文章内挑选最相关分片
- **工具**：只注册公开资料检索、深读、目录与 Wiki 工具及 Agent 元工具；不加载浏览器、沙箱、外部 MCP、写入、记忆或文档库能力；来源链接只指向 `/p/…` 与 `/wiki/…`
- **上下文**：访客只能提交普通 user/assistant 文本，历史最多 24 条、32000 字；可选「全部公开资料」或单个公开知识库（`/api/public/qa/scopes`）
- **历史**：服务端不落库；对话保存在访客浏览器，恢复时按 Agent 事件重放执行面板与来源

线上入口为 `/ask`，界面与站内助手共用消息、执行面板、引用条与输入框组件；配额与 SSE 契约保持兼容。
模型使用站长的 `CHAT` 配置，不代表获得站长的私有知识权限。
