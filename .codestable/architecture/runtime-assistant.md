...
- **子代理**（`tools/research-subagent.ts`）：`spawn_research_subagent` 嵌套只读 Agent（maxSteps≤6）；`depth/maxDepth` 默认 maxDepth=1（一层再委派，硬上限 2）；内层 step 前缀 `spawn_research_subagent` 或 `.../d{n}`；禁止写/危险。
