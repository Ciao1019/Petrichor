...
func ListWikiPagesForAgent(ctx context.Context, q Querier, userID, knowledgeBaseID int64) ([]map[string]any, error) {
...
func RunWikiLintForAgent(ctx context.Context, q Querier, userID, knowledgeBaseID int64) (map[string]any, error) {
	if _, err := assertKnowledgeBaseOwner(ctx, q, userID, knowledgeBaseID); err != nil {
		return nil, err
	}
	pages, err := loadWikiPageRows(ctx, q, userID, knowledgeBaseID)
	...
	return buildWikiLint(ctx, q, userID, knowledgeBaseID, pages)
}

// LoadWikiPageDetailForAgent 对应 wiki-agent-logic.ts loadWikiPageDetail。
func LoadWikiPageDetailForAgent(ctx context.Context, q Querier, userID, knowledgeBaseID int64, pageKey string) (map[string]any, error) {
	return wikiPageDetailCore(ctx, q, userID, knowledgeBaseID, pageKey)
}

// DeleteArticleWikiPagesForAgent 导出 deleteArticleWikiPages，供文章删除时清理 Wiki 派生数据。
func DeleteArticleWikiPagesForAgent(ctx context.Context, q Querier, userID int64, articles []ArticleRow, rebuildIndex bool) (int, error) {
	return deleteArticleWikiPages(ctx, q, userID, articles, rebuildIndex)
}

// LoadTreeNodesForAgent 导出 loadTreeNodes（PageIndex 目录树节点，供推理检索 / 语义检索）。
func LoadTreeNodesForAgent(ctx context.Context, q Querier, userID, knowledgeBaseID int64, articleID *int64) ([]TreeNodeRow, error) {
	return loadTreeNodesContext(ctx, q, userID, knowledgeBaseID, articleID)
}

// AssertKnowledgeBaseOwnerForAgent 归属校验（404 语义与 TS 一致）。
func AssertKnowledgeBaseOwnerForAgent(ctx context.Context, q Querier, userID, knowledgeBaseID int64) (*KBRow, error) {
	return assertKnowledgeBaseOwner(ctx, q, userID, knowledgeBaseID)
}

// QueryOwnedArticleForAgent 对应 handlers.ts loadOwnedArticle：不存在返回 nil（由调用方决定 404 文案）。
func QueryOwnedArticleForAgent(ctx context.Context, q Querier, userID, articleID int64) (*ArticleRow, error) {
