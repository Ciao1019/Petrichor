      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="flex min-w-0 flex-col gap-6">
          <div className="space-y-3">
            <div>
              <h2 className="text-base font-semibold">下载地址</h2>
          ...
          <div className="space-y-3">
            <div>
              <h2 className="text-base font-semibold">安装方式</h2>
              <p className="text-sm text-muted-foreground">选择客户端，下载 SKILL.md 并在运行 Agent 的环境中设置两个变量。</p>
            </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">单文件 Skill 只提供调用规则，不捆绑 CLI 或 config.json。</p>
            </div>
            <div className="divide-y divide-border/40 rounded-xl border border-border/40 bg-card/40">
              {SKILL_RULES.map((rule) => (
                <div key={rule.title} className="flex flex-col gap-1 px-4 py-3 sm:flex-row sm:gap-4">
        ...
        <div className="flex min-w-0 flex-col gap-6">
          <div className="space-y-3 rounded-xl border border-border/40 bg-muted/20 p-4">
            <h3 className="font-medium text-sm">使用步骤</h3>
            <AgentStepList
              steps={[
                {
                  title: (
          ...
          <div className="space-y-3 rounded-xl border border-border/40 bg-card/60 p-4">
            <div>
              <h3 className="font-medium text-sm">能力范围</h3>
              <p className="text-xs text-muted-foreground">单文件 Skill 引导 Agent 调用完整 REST 能力层。</p>
            </div>
