...
function SearchArticlesRender({ result, status }: { result: unknown; status?: ToolCallMessagePartStatus }) {
  const navigate = useNavigate()
  const payload = asRecord(result)
  const rows = Array.isArray(payload?.items) ? payload.items.map(asRecord).filter(isPresent) : []
  if (rows.length === 0) {
    return <ToolStatusCard title="检索公开文章" status={status} icon={<Search className="size-4" />} />
  }
  return (
    <ToolStatusCard title="检索公开文章" status={status} icon={<Search className="size-4" />} collapsible defaultOpen={false}>
      <div className="space-y-1.5">
        {rows.slice(0, 8).map((row, index) => {
          const href = typeof row.href === "string" ? row.href : ""
          const title = String(row.title ?? "公开文章")
